...
<div class="container">
            ...
            <div class="col-md-12">
                ...
                <form id="filter-form" method="GET">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <a href="{{ route('pembelian.printReportPDF', ['group' => $group, 'from_date' => $fromDate->toDateString(), 'to_date' => $toDate->toDateString()]) }} "
                    ...
                    <div class="d-flex mt-3">
                        ...
                        <select name="group" class="form-select form-select-sm me-2" id="group" onchange="$('#filter-form').submit()">
                            <option value="item" {{ $group == 'item' ? 'selected' : '' }}>Per Barang</option>
                            <option value="vendor" {{ $group == 'vendor' ? 'selected' : '' }}>Per Supplier</option>
                        </select>
    ...
    <div id="report-table-container">
        @if($group == 'item')
            @include('purchases.reports.partials.item_table', ['items' => $items])
        @else
            @include('purchases.reports.partials.vendor_table', ['pruchases' => $purchases])
        @endif
    </div>
</div>
@endsection
...
<script>
    $(document).ready(function () {
        ...
        $('#filter-form').on('submit', function (e) {
            ...
            var url = $(this).attr('action') || '{{ route("pembelian.reports") }}';
